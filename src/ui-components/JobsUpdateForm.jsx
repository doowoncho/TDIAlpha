...
import { updateJobs } from "../graphql/mutations";
...
export default function JobsUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setWo_number(cleanValues.wo_number);
    setPermit_number(cleanValues.permit_number);
    setPermit(cleanValues.permit);
    setPo_number(cleanValues.po_number);
    setMap(cleanValues.map);
    setPhoto(cleanValues.photo);
    setP_confirm(cleanValues.p_confirm);
    setPhone_number(cleanValues.phone_number);
    setNpat(cleanValues.npat);
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getJobs.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getJobs
        : jobsModelProp;
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateJobs.replaceAll("__typename", ""),
            variables: {
              input: {
                id: jobsRecord.id,
                ...modelFields,
